% 3D 实测 + 边缘拟合 + Vine Copula + BMA 加权 + 统计对比 + 相关性对比
    ...
    for f = families
        ...
        try
            switch fam
                ...
                case 'GEV'
                    pd = gevfit(x); p = 3;  % GEV分布参数估计
                case 'GPD'
                    % GPD拟合，参数为[k, sigma, theta]
                    [param_gpd, p_gpd] = gpdfit(x, theta_gpd); 
                    pd = param_gpd; 
                    p = p_gpd; % GPD有2个自由参数(k, sigma)
            end
...
for i = 1:length(unique_vine_orders)
    ...
    try
        % 使用修正版 Vine 拟合函数（不再使用 numerich 句柄）
...
%% 显示每种vine结构的tree1和tree2对应的优选copula函数
